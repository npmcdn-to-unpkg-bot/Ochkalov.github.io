$(function () {
    ...
    console.log('DB - таблицах',arrObjects);

    /*   1  */
    var arrSkills = _.map(arrObjects, 'skills');
    console.log('select distinct skills from ... order by 1',
        _.sortBy(_.uniq(_.flattenDeep(arrSkills)), function (i) { return i.toLowerCase(); } ) );


    /*2 */
    console.log('select name,count(friends) from ... order by 2',
        _.map(_.sortBy(arrObjects, function (o) { return o.friends.length; }), 'name'));


    /* 3 */
    console.log('select distinct name from ... ',
